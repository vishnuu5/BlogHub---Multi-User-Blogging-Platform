...
export default function NewPostPage() {
  ...
  return (
    <div className="container py-12 max-w-4xl">
      ...
      {error && <div className="mb-6 p-4 bg-red-100 border-2 border-red-300 text-red-800 rounded-lg">{error}</div>}

      <form onSubmit={handleSubmit} className="space-y-6 border-2 border-gray-300 rounded-lg p-6">
        <div>
        ...
        <div>
          <label className="block text-sm font-semibold mb-2 text-black">Categories</label>
          <div className="space-y-2 border-2 border-gray-200 rounded-lg p-4">
            {categories?.map((cat: any) => (
              <label key={cat.id} className="flex items-center gap-2 text-black">
                <input
                  type="checkbox"
